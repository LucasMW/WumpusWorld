...
seguro([X,Y]) :- adjacente([X,Y],[A,B]) , tem_cheiro([A,B],sim) , flechas(C), C > 0 .
...
agente_local(P), agente_orientacao(O), pontuacao(Score),flechas(A), write("P = "), write(P), write(" \nO = ") ,write(O), write(" \nScore = "),write(Score),write("\nflechas = "),write(A).
...
melhor_acao(fim) :-  (agente_vivo(nao), nl, write("lost")); (contagem_ouro(1), nl, write("won")),showAgent,!.
